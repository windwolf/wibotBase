...
void UART::_on_error_callback(UART_HandleTypeDef *instance) {
    ...
    //  if (perip->config.ignore_parity_error &&
    //      (instance->ErrorCode & HAL_UART_ERROR_PE)) {
    //    __HAL_UART_CLEAR_PEFLAG(instance);
    //    instance->ErrorCode &= (~HAL_UART_ERROR_PE);
    //  }
    //  if (perip->config.ignore_frame_error &&
    //      (instance->ErrorCode & HAL_UART_ERROR_FE)) {
    //    __HAL_UART_CLEAR_FEFLAG(instance);
    //    instance->ErrorCode &= (~HAL_UART_ERROR_FE);
    //  }
    //  if (perip->config.ignore_noise_error &&
    //      (instance->ErrorCode & HAL_UART_ERROR_NE)) {
    //    __HAL_UART_CLEAR_NEFLAG(instance);
    //    instance->ErrorCode &= (~HAL_UART_ERROR_NE);
    //  }
    //  if (perip->config.ignore_overrun_error &&
    //      (instance->ErrorCode & HAL_UART_ERROR_ORE)) {
...
#if PERIPHERAL_UART_READ_DMA_ENABLED
    //  return (Result)HAL_UART_Receive_DMA(
    //      &_handle, (uint8_t *)rxBuffer.data_ptr_get(),
