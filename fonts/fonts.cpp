...
namespace ww::graph {
static inline void setPixel(uint8_t *buffer, uint32_t memOffset,
                            uint32_t pixelSize, Color color) {
    if (pixelSize == PIXEL_SIZE_8BIT) {
        ...
        if (color.value == 1) {
            ((uint8_t *)buffer)[memOffset / 8] |= (uint8_t)(color.value)
                                                  << (7 - (memOffset % 8));
        } else {
...
bool FONTS_FillData(uint8_t *buffer, CanvasInfo *canvas, uint16_t x, uint16_t y,
    ...
    if (len == 0) {
        ...
        uint8_t fontMemRowSize =
            (fontInfo->direction == FONT_MEMORY_LAYOUT_DIRECTION_HORIZONTAL)
                ? (fontInfo->width + 7) / 8
                : (fontInfo->height + 7) / 8;

        uint8_t fontMemWidth =
            (fontInfo->direction == FONT_MEMORY_LAYOUT_DIRECTION_HORIZONTAL)
                ? fontInfo->width
                : fontInfo->height;
        uint8_t fontMemHeight =
            (fontInfo->direction == FONT_MEMORY_LAYOUT_DIRECTION_HORIZONTAL)
                ? fontInfo->height
                : fontInfo->width;
        ...
        uint8_t memX =
            canvas->direction == CANVAS_MEMORY_LAYOUT_DIRECTION_HORIZONTAL ? x
                                                                           : y;
        uint8_t memY =
            canvas->direction == CANVAS_MEMORY_LAYOUT_DIRECTION_HORIZONTAL ? y
                                                                           : x;
        uint8_t canvasMemWidth =
            canvas->direction == CANVAS_MEMORY_LAYOUT_DIRECTION_HORIZONTAL
                ? canvas->width
                : canvas->height;
        // uint8_t canvasMemHeight = canvas->direction ==
        // CANVAS_MEMORY_LAYOUT_DIRECTION_HORIZONTAL ? canvas->height :
        // canvas->width;
        for (uint16_t i = 0; i < len; i++) {
            ...
            if (i != 0) {
                ...
                for (uint16_t j = 0; j < lineSpacingMemHeight; j++) {
                    ...
                    for (uint16_t k = 0; k < lineSpacingMemWidth; k++) {
                        ...
                        setPixel(buffer, memOffset, pixelSize,
                                 fontDrawInfo->backColor);
                    }
                }
            ...
            const uint8_t *fontData =
                &(fontInfo->table[(str[i] - 32) * fontMemSize]);
            ...
            for (uint16_t j = 0; j < fontMemHeight; j++) {
                ...
                for (uint16_t k = 0; k < fontMemWidth; k++) {
                    ...
                    Color color;
                    if (fontData[j * fontMemRowSize + k / 8] &
                        (1 << (fontMemWidth - k - 1))) {
